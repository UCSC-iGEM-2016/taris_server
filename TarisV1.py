...
class Taris_SW:
    ...
    #Appends JSON information into a list
    cucumber = getHistoryData()
    historyLog.append(cucumber)

    @app.route('/dataRetrieve', methods=['POST'])
    def dataHold():
        pickler = Taris_SW.historyLog
        #will json.dumps still work for
        requests.post(url, json.dumps(pickler))
